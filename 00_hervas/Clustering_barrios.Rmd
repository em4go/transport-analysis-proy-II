...
Para finalizar la interpretación de los clusters vamos a sacar el porcentaje de barrios de cada cluster que tienen alguno de los servicions especiales (Universidad, Estadio, Hospitales, Cargadores) que hemos seleccionado en el análisis. Ya que no hemos podido tenerlos en cuenta.

```{r}
dataBarrios3$cluster<-ward5
dataBarrios3$Especial<-ifelse(rowSums(dataBarrios[, c("Universidad", "Estadio", "Hospitales", "Cargadores")]) > 0, 1, 0)
porcentaje_cluster <- aggregate(Especial ~ cluster, data = dataBarrios3, FUN = function(x) mean(x == 1) * 100)
porcentaje_cluster
```
Podemos observar como al igual que en el perfil medio de los clusters, el cluster 4 es el que presenta un mayor porcentaje de barrios con servicios especiales. Por otro lado, el cluster 3 es el que presenta un menor porcentaje de barrios con servicios especiales. Del cluster 5 no se puede sacar una conclusión clara, ya que solo contiene un barrio.
...
centroides <- lapply(dataBarrios3$geometry, function(poly) {
  centro <- st_centroid(poly)
  return(c(x = st_coordinates(centro)[1], y = st_coordinates(centro)[2]))
})

# Crear un dataframe con los centroides
centroides2 <- data.frame(lon = sapply(centroides, "[[", "x"),
                             lat = sapply(centroides, "[[", "y"))

centroides3<-centroides2[dataBarrios3$Especial==1,]
lista_iconos <- icons(iconUrl = 'https://cdn.icon-icons.com/icons2/3002/PNG/512/pushpin_icon_187849.png', iconWidth = c(20), iconHeight = c(20))
m2 <- leaflet() %>%
  addTiles() %>%
  ...
  addMarkers(data = centroides3, popup = rownames(dataBarrios3), icon=lista_iconos)
m2
m <- leaflet() %>%
  addTiles() %>%
  addPolygons(data = dataBarrios3, fillColor = ~colors_cluster(cluster), fillOpacity = 1, weight = 0.5, color = "black",
...
El cluster 4 (azul), era el cluster que agrupaba los barrios con un nivel de vida más costoso, y con mayor número de servicions disponible y efectivamente, estos barrios se encuentran en la zona más céntrica de la ciudad, donde el nivel de vida es más elevado y hay una gran afluencia.
...
m2 <- leaflet() %>%
 ...
 m <- m %>% addMarkers(lng = X, lat = Y, popup = gtfs.stop[gtfs.stop$distance_to_start < dist,]$station_name, icon = icon)
```
