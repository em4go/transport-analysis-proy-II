...
Se han probado todo los modelos jerárquicos disponibles en R, (single,complete,average y ward), y pese a que todos mostraban resultados aceptables, hemos seleccionado los 2 más completos. Por lo que solo mostraremos los resultados de los métodos Ward y Complete.
...
fviz_cluster(object = list(data=dataMetros2, cluster=complete7), stand = FALSE,
  ...
  labs(title = "Modelo jerarquico + Proyeccion PCA",
       subtitle = "Dist manhattan, Metodo comlete, K=5") +
  theme_bw() +
  theme(legend.position = "bottom")
```
...
valencia<- geojson_sf(data_geo$geo_shape)
...
```{r}
m2 <- leaflet() %>%addProviderTiles(providers$CartoDB.Positron)  %>% setView(lng = -0.375, lat = 39.4667, zoom = 12)
m2 <- m2 %>%
  addPolygons(data = valencia, fillColor = "transparent", weight = 2, color = "#2F4F4F",dashArray = "3, 7")
m2 <- plot_graph(m2, dataMetros3, dataMetros3$cluster)
m2
```

Al observar las paradas de metro en el mapa de Valencia, podemos observar que el las paradas del cluster verde, que tenian valores bajos en todas las variables, se encuentran en su mayoria fuera de la ciudad de Valencia, y suelen ser paradas de fin de linea, lo que explica su bajo número de conexiones asi como importancia en la red.
En cuanto a las paradas del cluster amarillo, que presentaban valores intermedios en todas las variables, son las paradas más comunes en la red, y se encuentran por repartidas por toda la zona, excepto las zonas más centrales. Además, no son paradas donde se separen las lineas.
Las nodos del cluster azul claro, caracterizado por valores intermedios pero con mayor centralidad y capacidad. Se reparten de forma muy similar a los nodos del cluster amarillo, pero se encuentran más centrados en la red, no tan cerca de los finales de linea.
Las paradas del cluster rojo, caracterizadas por tener muchos vecinos pero poca centralidad, se encuentran en puntos donde se separa la línea, por lo que son importantes en la red, pero en su mayoría no son paradas muy centrales.
El cluster azul oscuro, que presenta valores altos en todas las variables, pero con menos importancia en la red, debido a que se encuentran en las zonas más centrales de la red y son los nodos con más capacidad y afluencia en la red, pero no son nodos donde se crucen muchas líneas.
Por últimos, las paradas del cluster morado, que presentan valores altos en todas las variables, observamos que no son tan centrales como las del cluster anterior pero siguen siendolo, y a diferencia de estos, están mucho más conectadas y son los nodos más importantes en la red, ya que son los nodos donde se cruzan más líneas.

```{r}
saveRDS(m2, "../00_marc/app_shiny_v1/app_shiny/mapas/cluster_metros.rds")
```

```{r}
#write_parquet(dataMetros3, "cluster_metro.parquet")
```
