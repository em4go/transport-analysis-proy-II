...
También se han dividido todas las variables numéricas por su superficie para que puedan ser comparadas entre los barrios, ya que, de no ser así, podría haber barrios con valores más elevados tan solo porque abarcan una mayor superficie.

```{r base_final}
df_final <- df2[which(df2$distrito != "POBLATS DEL NORD" & df2$distrito != "POBLATS DEL SUD"),]
df_numerico <- df_final %>% select(-c("barrio", "distrito", "geo_shape", "precio_alquiler"))
#Dividimos las variables por la superficie
p <- df_numerico %>% apply(2, function(x) x / df_numerico$superficie_barrio)
p <- data.frame(p) %>% select(-superficie_barrio)

df_final_sup <- cbind(barrio = df_final$barrio, p, distrito = df_final$distrito,
                  superficie_barrio = df_final$superficie_barrio, precio_alquiler = df_final$precio_alquiler)
```


# Creación de los índices

Vamos a proceder a crear distintos índices que caractericen a cada barrio.

## Cantidad de servicios ofrecidos por $m^2$
...
```{r write_data}
write_parquet(df_final_sup, "../data/info_general_barrio_final.parquet")
write.csv(df_final_sup, "../data/info_general_barrio_final.csv")


write_parquet(df_final, "../data/info_general_barrio.parquet")
write.csv(df_final, "../data/info_general_barrio.csv")
summary(df_final_sup)
```
