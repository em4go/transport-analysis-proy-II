...
plot_graph <- function(dataset,color, tamanyo, g_fun){
  ...
  mapa <- plot_isochron(g_fun)
  mapa <- mapa %>%
    addCircleMarkers(data = b, color = ~colors(color), radius = ~(tamanyo*70)^2 , fillOpacity = 1,
...
plot_graph(carac_metro, color = carac_metro$pagerank, tamanyo = carac_metro$closeness, g_fun = g)
```
