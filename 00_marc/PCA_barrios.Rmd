...
dataBarrios2 <- subset(dataBarrios, select = -c(barrio, geo_shape, X, X0_15_años, X16_64_años, X65_o_más))
```
...
```{r}
#Creación del PCA
res.pca = PCA(dataBarrios2, scale.unit = TRUE, ncp = Inf, graph = FALSE, quali.sup = "distrito")
...
```{r}
res.pca = PCA(dataBarrios2, scale.unit = TRUE, ncp = K, graph = FALSE, quali.sup = "distrito")
```
...
barriosCE = subset(dataBarrios2, select=-c(distrito))
...
```{r}
res.pca1 = PCA(barrios_sin_lapunta, scale.unit = TRUE, ncp = 4, graph = FALSE, quali.sup = "distrito")
res.pca2 = PCA(barrios_sin_grandes, scale.unit = TRUE, ncp = 4, graph = FALSE, quali.sup = "distrito")
res.pca3 = PCA(barrios_sin_anomalos, scale.unit = TRUE, ncp = 4, graph = FALSE, quali.sup = "distrito")
...
```{r}
res.pca = PCA(barrios_sin_lapunta, scale.unit = TRUE, ncp = 4, graph = FALSE, quali.sup = "distrito")
```
